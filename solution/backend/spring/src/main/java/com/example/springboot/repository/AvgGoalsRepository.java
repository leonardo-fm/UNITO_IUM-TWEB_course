...
public interface AvgGoalsRepository extends JpaRepository<AvgGoals, Integer> {

    @Query(value = "SELECT season as year, game.competition_id, " +
            "AVG(CASE " +
            "WHEN home_club_id = :clubId THEN home_club_goals " +
            "WHEN away_club_id = :clubId THEN away_club_goals " +
            "ELSE 0 END) AS avg_goals " +
            "FROM game WHERE home_club_id = :clubId OR away_club_id = :clubId GROUP BY season, game.competition_id;", nativeQuery = true)
